# Claude Code Project Instructions — icu-locale-core-kotlin

## Project Overview

This is **icu-locale-core-kotlin**, a clean-room Kotlin Multiplatform port of the upstream Rust crate [`icu_locale_core`](https://crates.io/crates/icu_locale_core).

The upstream Rust source is the read-only translation oracle. When porting begins, clone it into `tmp/icu_locale_core/` (gitignored). **Never edit `tmp/`.** If upstream looks wrong, the bug is in the port or in your understanding of Rust, not in `tmp/`.
...
```kotlin
// port-lint: source <path-relative-to-tmp/icu_locale_core>
